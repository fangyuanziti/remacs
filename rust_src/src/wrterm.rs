...
use crate::{
    ...
    remacs_sys::{
        ...
        Qminibuffer, Qname, Qnil, Qparent_id, Qt, Qterminal, Qunbound, Qwr, Qx, Qx_create_frame_2,
        Vframe_list, WRImage, Window, XColor, XrmDatabase, DEFAULT_REHASH_SIZE,
...
pub fn x_create_frame(parms: LispObject) -> LispFrameRef {
    ...
    let output_size = output.get_inner_size().expect("Output size not existed!");

    // FIXME: text size and pixel size are different.
    frame.text_width = output_size.width as i32;
    frame.pixel_width = output_size.width as i32;
    frame.text_height = output_size.height as i32;
